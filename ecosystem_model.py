...
class EcosystemModel:
    ...
    @tf.function
    def step(self):
        # 1. CONVOLUTION (Get Neighbors)
        # We need this to allow plants to spread (Dispersal)
        current_grid = self.grid
        padded = tf.pad(current_grid, [[1, 1], [1, 1], [0, 0]])
        padded_batch = padded[tf.newaxis, :, :, :]
        
        # This sums the 3x3 neighborhood for EVERY channel
        ...
        # Neighbor Pools (for Dispersal)
        neigh_flat = neighbors[:, :, 4:pool_end]
        neigh_pools = tf.reshape(neigh_flat, (self.H, self.W, self.N_SPP, 5))
        
        # Extract Carbon (Index 0)
        spp_C = spp_pools[:, :, :, 0]       # Current Cell Carbon
        neigh_C = neigh_pools[:, :, :, 0]   # Sum of 3x3 Neighbor Carbon
        ...
        # 4. Interactions (Space Crowding)
        space_factor = tf.maximum(0.0, 1.0 - (total_biomass / self.params['K']))
        space_factor = tf.expand_dims(space_factor, -1)
        
        # 5. Colonization (Seed Rain)
        ...
        # 7. DISPERSAL (Spatial Spread)
        # Plants spread into empty space based on neighbor abundance
        # Rate: 1% of neighbor sum (avg of 3x3)
        # Only disperse if space is available (space_factor)
        # Note: neigh_C is SUM of 9 neighbors. Average is neigh_C / 9.
        dispersal_rate = 0.01 
        dispersal = (neigh_C / 9.0) * dispersal_rate * space_factor
        
        # Combine: Old + Seed + Growth + Dispersal
        spp_C_new = spp_C_seeded + growth + dispersal
        
        # 8. Nutrient Demands (Vectorized)
