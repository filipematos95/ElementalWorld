...
class HybridEcosystem:
    def __init__(self, height=100, width=100, max_agents=100000):
        ...
        # [4:C, 5:N, 6:P, 7:K, 8:O] (Current Internal Elementome)
        # [9:alive]
        ...
        self.K_biomass           = 1.5   # Space Carrying Capacity (Max Biomass per pixel)

    def add_initial_seeds(self, count=50, niche_center=None):
        """
        Add seeds.
        If niche_center is provided (list of 5 floats), agents start with that stoichiometry.
        If None, they are random.
        """
        ...
        # 1. GENERATE STOICHIOMETRY (Initial Internal Stores)
        if niche_center is not None:
            center_tensor = tf.convert_to_tensor(niche_center, dtype=tf.float32)[tf.newaxis, :]
            noise = tf.random.normal((count, 5), mean=0.0, stddev=0.05)
            raw = center_tensor + noise
        ...
        print(f"Added {count} seeds. Total Agents: {self.n_agents.numpy()}")
    ...
    def step(self):
        ...
        # ==========================================
        # PHASE 2: AGENT BIOLOGY (DYNAMIC ELEMENTOME)
        # ==========================================
        ...
        # CURRENT STATUS (Internal Concentration)
        ...
        # Demand (Try to fill capacity, limited by root speed)
        uptake_rate = 0.5
        ...
        # Update Living Agents (with NEW dynamic elementome)
        up_rows = tf.concat([
            active_data[:, 0:3],
            fin_mass[:, tf.newaxis],
            active_data[:, 4:5],
        ...
        if n_s > 0:
            p_dat = tf.gather(up_rows, p_idx) # Inherit UPDATED state
            ...
            c_rows = tf.concat([
                ...
                p_dat[:, 4:9], # Maternal Effect: Inherit parent's current stoich
                tf.ones((n_s, 1))
            ], axis=1)
